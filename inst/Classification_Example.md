...
``` r
if (!("gdata" %in% installed.packages()[,"Package"])){
...
if (!("CORElearn" %in% installed.packages()[,"Package"])){
  install.packages("CORElearn", repos = "http://cran.us.r-project.org", dependencies = TRUE)
}
library(CORElearn)
...
if (!("SDMTools" %in% installed.packages()[,"Package"])){
  install.packages("SDMTools", repos = "http://cran.us.r-project.org", dependencies = TRUE)
}
library(SDMTools)
...
if (!("RMariaDB" %in% installed.packages()[,"Package"])){
  install.packages("RMariaDB", repos = "http://cran.us.r-project.org", dependencies = TRUE)
}
library(RMariaDB)
...
Plot of classification accuracy associated with five methods, PageRank(PK), Katz, Epistasis-Katz(EK), EpistasisRank(ER), and XGBoost (xgb). Methods in above panel use co-expression network and methods in bottom panel use the reGAIN network. Also, accuracies at the left side don't use prior knowledge and accuracies at right side use the prior knowledge.

``` r
grid.arrange(p1, p2, nrow = 2)
```

![](Classification_Example_files/figure-markdown_github/unnamed-chunk-18-1.png)
