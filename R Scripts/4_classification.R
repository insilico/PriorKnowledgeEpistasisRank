...
# Epistasis-Katz and Rinbix::EpistasisRank centrality methods and xgboost classifier with and without prior knowledge.
#
...
for (i in 1:5){
  ...
  for (j in 1:length(inner_folds)){
    ...
    while (ncol(camb_expr.fltr_cov)>200){
      camb_expr.fltr_cov <- t(Rinbix::geneLowCoefOfVarFilter(exprData,threshold = thresh)$fdata)
